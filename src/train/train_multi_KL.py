...
from trainer import STrainer
from transformers import AutoConfig, AutoTokenizer, HfArgumentParser, set_seed
from other_arguments import otherArguments
from model import SModel
from data_collator import QPCollator
from trainer import NewDataset


logger = logging.getLogger(__name__)
def main():
    parser = HfArgumentParser((ModelArguments, DataArguments, TrainingArguments,otherArguments))

    if len(sys.argv) == 2 and sys.argv[1].endswith(".json"):
        model_args, data_args, training_args, other_args = parser.parse_json_file(json_file=os.path.abspath(sys.argv[1]))
    else:
    ...
    logger.info("OTHER parameters %s", other_args)
    ...
    trainer = trainer_cls(
        ...
        data_collator=QPCollator(
            ...
            max_sl_len= other_args.sl_max_len,
        ),
